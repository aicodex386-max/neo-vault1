...
## Edit (single image)
\`\`\`bash
uv run scripts/generate_image.py --prompt "edit instructions" --filename "output.png" -i "/path/in.png" --resolution 2K
\`\`\`
...
- Or set \`skills."nano-banana-pro".apiKey\` in \`~/.openclaw/openclaw.json\`

## Specific aspect ratio (optional)
\`\`\`bash
uv run scripts/generate_image.py --prompt "portrait photo" --filename "output.png" --aspect-ratio 9:16
\`\`\`
